 ...
 */
import { computed, toValue, toRaw } from 'vue';
import { encryptWith } from '../actions/encrypt.js';
 ...
 * ```
 */
export function useFHEEncryption(parameters) {
    ...
    const encryptWithFn = async (buildFn) => {
        const instanceValue = toValue(instance);
        const signerValue = toValue(ethersSigner);
        const contractValue = toValue(contractAddress);
