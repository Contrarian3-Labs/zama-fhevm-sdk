 ...
 */
export async function decrypt(config, parameters) {
    ...
    try {
        ...
        if (!sig) {
            throw new Error('SIGNATURE_ERROR: Failed to create or load decryption signature');
        }
        // Validate signature is not expired
        if (!sig.isValid()) {
            // Clear expired signature from storage
            throw new Error('SIGNATURE_EXPIRED: Decryption signature has expired. Please refresh and try again.');
        }
        // Validate signature covers all requested contract addresses
        const hasAllAddresses = uniqueAddresses.every((addr) => sig.contractAddresses.includes(addr));
        if (!hasAllAddresses) {
            throw new Error('SIGNATURE_MISMATCH: Cached signature does not cover all requested contracts. ' +
        ...
        const results = await instance.userDecrypt(mutableReqs, sig.privateKey, sig.publicKey, sig.signature.replace('0x', ''), sig.contractAddresses, sig.userAddress, sig.startTimestamp, sig.durationDays);
        return results;
    }
    catch (error) {
        /**
 ...
 */
export async function getDecryptionSignature(config, parameters) {
