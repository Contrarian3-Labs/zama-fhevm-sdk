 ...
 */
export async function createInstance(config, parameters) {
    ...
    const { isMock, rpcUrl, chainId } = await resolve(providerOrUrl, config.mockChains);
    // Validate chainId is configured
    const isConfigured = config.chains.includes(chainId) ||
        (config.mockChains && Object.hasOwn(config.mockChains, chainId));
    if (!isConfigured) {
        throw new FhevmError('CHAIN_NOT_CONFIGURED', `Chain ${chainId} is not configured in FHEVM config. ` +
    ...
    try {
        ...
        if (typeof window === 'undefined') {
            throw new FhevmError('SSR_NOT_SUPPORTED', 'Production FHEVM instances require browser environment (window.relayerSDK). Use mock chains for SSR.');
        }
        if (!isFhevmWindowType(window, console.log)) {
            // Load SDK
            await fhevmLoadSDK();
        ...
        if (chainId !== 11155111) {
            console.warn(`[createInstance] Using SepoliaConfig for chainId ${chainId}. ` +
        ...
        if (!checkIsAddress(aclAddress)) {
            throw new FhevmError('INVALID_ACL_ADDRESS', `Invalid ACL contract address: ${aclAddress} for chain ${chainId}`);
        }
