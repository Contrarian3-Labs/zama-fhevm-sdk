 ...
 */
export const getEncryptionMethod = (type) => {
    ...
    const normalized = type.replace(/^external/i, '').toLowerCase();
    switch (normalized) {
        case 'ebool':
            return 'addBool';
        case 'euint8':
        ...
        case 'eaddress':
            return 'addAddress';
        default:
            throw new Error(`Unknown encryption type: "${type}" (normalized: "${normalized}"). ` +
 ...
 */
export async function encrypt(config, parameters) {
    ...
    for (const { type, value } of values) {
        ...
        if (typeof input[method] !== 'function') {
            throw new Error(`Invalid encryption method: ${method} for type: ${type}. ` +
        ...
        try {
            ...
            throw new Error(`Failed to encrypt value of type ${type}: ${error.message}`, { cause: error });
        }
    }
    // Encrypt and return
 ...
 */
export async function encryptWith(config, parameters) {
